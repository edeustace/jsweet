...
export class StackTraceCreator {
  ...
  static __static_initialize() {
    if (!StackTraceCreator.__static_initialized) {
      StackTraceCreator.__static_initialized = true;
      StackTraceCreator.__static_initializer_0();
    }
  }
  ...
  public static CollectorLegacy = class CollectorLegacy extends StackTraceCreatorNamespace.Collector {
    ...
    public getStackTrace(t: any): StackTraceElement[] {
      ...
      const stackTrace: StackTraceElement[] = ((s) => {
        let a = [];
      ...
      for (let i: number = 0; i < length; i++) {
        {
          stackTrace[i] = new StackTraceElement(
            StackTraceCreator.UNKNOWN,
            stack.get(i),
            null,
            StackTraceCreator.LINE_NUMBER_UNKNOWN,
          );
        }
      }
      return stackTrace;
  ...
  public static CollectorEmulated = class CollectorEmulated extends StackTraceCreatorNamespace.Collector {
    /**
     *
     * @param {*} error
     */
     ...
     */
    public getStackTrace(t: any): StackTraceElement[] {
      const stack: JsArray<JsArrayString> = <any>(
        StackTraceCreator.getFnStack(t).cast<any>()
      );
      const stackTrace: StackTraceElement[] = ((s) => {
        let a = [];
        while (s-- > 0) a.push(null);
      ...
      for (let i: number = 0; i < stackTrace.length; i++) {
        {
          ...
          if (location != null) {
            ...
            if (idx !== -1) {
              ...
              lineNumber = StackTraceCreator.parseInt(
  ...
  public static CollectorModern = class CollectorModern extends StackTraceCreatorNamespace.Collector {
    /**
     *
     * @param {*} error
     */
    ...
    parse(stString: string): StackTraceElement {
      ...
      if (/* isEmpty */ stString.length === 0) {
        return this.createSte(
          StackTraceCreator.UNKNOWN,
          StackTraceCreator.ANONYMOUS,
          StackTraceCreator.LINE_NUMBER_UNKNOWN,
          -1,
        );
      }
      ...
      if (
        /* isEmpty */ toReturn.length === 0 ||
        toReturn === ieAnonymousFunctionName
      ) {
        toReturn = StackTraceCreator.ANONYMOUS;
      }
      const lastColonIndex: number = location.lastIndexOf(":");
      const endFileUrlIndex: number = location.lastIndexOf(
        ":",
        lastColonIndex - 1,
      );
      ...
      if (lastColonIndex !== -1 && endFileUrlIndex !== -1) {
        ...
        line = StackTraceCreator.parseInt(
          location.substring(endFileUrlIndex + 1, lastColonIndex),
        );
        col = StackTraceCreator.parseInt(
          location.substring(lastColonIndex + 1),
        );
      }
    ...
    createSte(
      ...
      return new StackTraceElement(
        ...
        method,
        fileName + "@" + col,
        line < 0 ? StackTraceCreator.LINE_NUMBER_UNKNOWN : line,
      );
    }
  ...
  public static CollectorNull = class CollectorNull extends StackTraceCreatorNamespace.Collector {
    /**
     *
     * @param {*} error
     */
  ...
  public static constructJavaStackTrace(thrown: Error): StackTraceElement[] {
    const stackTrace: StackTraceElement[] =
      StackTraceCreator.collector_$LI$().getStackTrace(thrown);
    return StackTraceCreator.dropInternalFrames(stackTrace);
  }

  static dropInternalFrames(
    stackTrace: StackTraceElement[],
  ): StackTraceElement[] {
    const dropFrameUntilFnName: string = Impl.getNameOf(
      "@com.google.gwt.core.client.impl.StackTraceCreator::captureStackTrace(*)",
    );
    const dropFrameUntilFnName2: string = Impl.getNameOf(
      "@java.lang.Throwable::initializeBackingError(*)",
    );
    const numberOfFramesToSearch: number = Math.min(
      stackTrace.length,
      StackTraceCreator.DROP_FRAME_LIMIT,
    );
    for (let i: number = numberOfFramesToSearch - 1; i >= 0; i--) {
      {
        if (
          stackTrace[i].getMethodName() === dropFrameUntilFnName ||
          stackTrace[i].getMethodName() === dropFrameUntilFnName2
        ) {
  ...
  static collector: StackTraceCreator.Collector;
  public static collector_$LI$(): StackTraceCreator.Collector {
    StackTraceCreator.__static_initialize();
    return StackTraceCreator.collector;
  }

  static __static_initializer_0() {
    const enforceLegacy: boolean = !StackTraceCreator.supportsErrorStack();
    const c: StackTraceCreator.Collector = <any>(
      GWT.create<any>(StackTraceCreator.Collector)
    );
    StackTraceCreator.collector =
      c != null &&
      c instanceof <any>StackTraceCreator.CollectorModern &&
      enforceLegacy
  ...
  static getFunctionName(fn: JavaScriptObject): string {
    return (
      fn.name ||
      (fn.name = StackTraceCreator.extractFunctionName(fn.toString()))
    );
  }
